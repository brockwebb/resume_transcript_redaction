...
def display_gui():
    ...
    try:
        processor = RedactionProcessor(
            ...
            detection_patterns_path=config_loader.config_files.get('detection', 'detection_patterns.yaml'),
            trigger_words_path=config_loader.config_files.get('word_filters', 'custom_word_filters.yaml'),
    ...
    with col2:
        current_trigger_words = ', '.join(word_filters.get('trigger_words', []))
        manual_trigger_words = st.text_area("Trigger Words", 
                                          value=current_trigger_words,
                                          placeholder="Enter trigger words (comma-separated)")
        if st.button("Update Trigger Words"):
            new_trigger_words = [word.strip() for word in manual_trigger_words.split(",") if word.strip()]
            processor.trigger_words.update(new_trigger_words)
            st.success(f"Added {len(new_trigger_words)} trigger words.")
            ...
            if not pdf_files:
                ...
                for idx, pdf_file in enumerate(pdf_files, 1):
                    ...
                    try:
                        ...
                        total_stats["total_redacted"] += stats.get("redacted_words", 0)
